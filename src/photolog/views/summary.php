...
<table class="table table-sm" id="<?= $uid ?>" data-role="photolog-table">
	<thead class="small">
		<tr>
			...
			<td class="d-none d-md-table-cell text-center" data-role="sort-header" data-key="files" data-sorttype="numeric" style="width: 17%;">files</td>
...
<?php	}	// foreach( $this->data->dto as $dto) {	?>
	...
	<?php if ( $this->data->dtoSet) {	?>
		...
		<tfoot>
			...
			<?php	if ( $totFiles > 0) {	?>
				<tr>
					<td colspan="3"><em class="text-muteds  small">Average File Size: <?php
						$av = $totSize / $totFiles;
						if ( $av > 1024000) {
